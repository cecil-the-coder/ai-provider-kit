...
func TestOpenAIParser_Parse(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			model: "gpt-4",
			want: MakeTestInfo("openai", "gpt-4",
				WithRequests(60, 58, ""),
				WithTokens(90000, 85000, ""),
		...
		},
		{
			...
			model: "gpt-3.5-turbo",
			want: MakeTestInfo("openai", "gpt-3.5-turbo",
				WithRequests(100, 95, ""),
			),
			wantErr: false,
		},
		{
			...
			model: "gpt-4-turbo",
			want: MakeTestInfo("openai", "gpt-4-turbo",
				WithTokens(150000, 140000, ""),
			),
			wantErr: false,
		},
		{
			...
			want: MakeTestInfo("openai", "gpt-4",
				WithRequests(60, 0, ""),
				WithRetryAfter(300*time.Second),
			),
			wantErr: false,
		},
		{
